...
namespace RestoWs
{
    public class WorkerV2 : BackgroundService
    {
        ...
        private static string NAMA_PAJAK = "RESTORAN";
        private static EnumFactory.EPajak PAJAK_ENUM = EnumFactory.EPajak.MakananMinuman;
...
WHERE fk_pajak_daerah = '02' and status_op != 0 and kategori_pajak = 'RESTORAN'
                ...
                int index = 0;
                var newList = new List<MonPDLib.EF.DbOpResto>();
                var removeEx = _contMonPd.DbOpRestos.Where(x => x.TahunBuku == tahunBuku).ToList();
                //var updateList = new List<MonPDLib.EF.DbOpResto>();
                foreach (var item in result)
                {
                    ...
                    Console.Write($"\r[{tglMulai.ToString("dd MMM yyyy HH:mm:ss")}] OP RESTORAN TAHUN {tahunBuku} JML OP {jmlData.ToString("n0")} Baru: {newList.Count.ToString("n0")} [({persen:F2}%)]");
                }

                Console.WriteLine("Updating DB!");
                if (removeEx.Any())
                {
                    _contMonPd.DbOpRestos.RemoveRange(removeEx);
                    _contMonPd.SaveChanges();
                }
                if (newList.Any())
                {
                    _contMonPd.DbOpRestos.AddRange(newList);
                    _contMonPd.SaveChanges();
                }
...
GROUP BY NOP, MASA_PAJAK, TAHUN_PAJAK,SEQ                                            
                ...
                if (newList.Any())
                {
                    _contMonPd.DbMonRestos.AddRange(newList);
                    _contMonPd.SaveChanges();
                }
