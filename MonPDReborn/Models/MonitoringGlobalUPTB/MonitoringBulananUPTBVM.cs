    ...
    public class MonitoringBulananUPTBVM
    {
        ...
        public class Show
        {
            ...
            public Show(EnumFactory.EPajak jenisPajak, int wilayah, int tahun)
            {
                ...
                AkumulasiBulananPajakList = Method.GetDataBulananPajak(jenisPajak, wilayah, tahun);
                if ((int)jenisPajak != 0)
                {
        ...
        public class Method
        {
            public static List<MonitoringBulananViewModels.BulananPajak> GetDataBulananPajak(EnumFactory.EPajak jenisPajak, int wilayah, int tahun)
            {
                ...
                switch (jenisPajak)
                {
                    ...
                    default:
                        ...
                        realisasiSemuaPerBulan.AddRange(
                            context.DbMonPbbs
                                .Where(x => x.TglBayar.HasValue && x.TglBayar.Value.Year == tahun && x.TahunBuku == tahun && nopPbb.Contains(x.Nop))
                                .GroupBy(x => x.TglBayar.Value.Month)
                                .Select(g => new ValueTuple<int, decimal>(g.Key, g.Sum(x => x.JumlahBayarPokok) ?? 0))
                                .ToList()
                        );
            ...
            /*public static MonitoringBulananViewModels.DataRekapBulanan GetDataRekapBulanan(EnumFactory.EPajak jenisPajak, int wilayah, int tahun)
            {
                var ret = new MonitoringBulananViewModels.DataRekapBulanan();
                var context = DBClass.GetContext();
