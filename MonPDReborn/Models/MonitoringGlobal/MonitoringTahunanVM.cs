    ...
    public class MonitoringTahunanVM
    {
        ...
        public class Method
        {
            public static List<MonitoringTahunanViewModels.TahunanPajak> GetTahunanPajak(DateTime TanggalCutOff)
            {
                ...
                //var dataRealisasiPbb = context.DbMonPbbs.Where(x => x.TglBayarPokok.Value.Year == TanggalCutOff.Year && x.TglBayarPokok.Value >= new DateTime(TanggalCutOff.Year, 1, 1) && x.TglBayarPokok.Value <= TanggalCutOff).Sum(x => x.NominalPokokBayar) ?? 0;
                ...
                var dataRealisasiAbt = context.DbMonAbts.Where(x => x.TglBayarPokok.Value.Year == TanggalCutOff.Year && x.TglBayarPokok.Value >= new DateTime(TanggalCutOff.Year, 1, 1) && x.TglBayarPokok.Value <= TanggalCutOff).Sum(x => x.NominalPokokBayar) ?? 0;
                ...
                var listTahunan = new List<TahunanPajak>
                    {
                        new() { JenisPajak = EPajak.MakananMinuman.GetDescription(), AkpTahun = dataTargetMamin, RealisasiSDHariAccrual = dataRealisasiMamin, PersenAccrual = dataTargetMamin > 0 ? Math.Round((decimal)dataRealisasiMamin / dataTargetMamin * 100, 2) : 0 },
                        new() { JenisPajak = EPajak.TenagaListrik.GetDescription(),  AkpTahun = dataTargetListrik, RealisasiSDHariAccrual = dataRealisasiListrik, PersenAccrual = dataTargetListrik > 0 ? Math.Round((decimal)dataRealisasiListrik / dataTargetListrik * 100, 2) : 0 },
                        new() { JenisPajak = EPajak.JasaPerhotelan.GetDescription(), AkpTahun = dataTargetHotel, RealisasiSDHariAccrual = dataRealisasiHotel, PersenAccrual = dataTargetHotel > 0 ? Math.Round((decimal)dataRealisasiHotel / dataTargetHotel * 100, 2) : 0 },
                        new() { JenisPajak = EPajak.JasaKesenianHiburan.GetDescription(), AkpTahun = dataTargetHiburan, RealisasiSDHariAccrual = dataRealisasiHiburan, PersenAccrual = dataTargetHiburan > 0 ? Math.Round((decimal)dataRealisasiHiburan / dataTargetHiburan * 100, 2) : 0 },
                        new() { JenisPajak = EPajak.JasaParkir.GetDescription(),     AkpTahun = dataTargetParkir, RealisasiSDHariAccrual = dataRealisasiParkir, PersenAccrual = dataTargetParkir > 0 ? Math.Round((decimal)dataRealisasiParkir / dataTargetParkir * 100, 2) : 0 },
                        new() { JenisPajak = EPajak.Reklame.GetDescription(),        AkpTahun = dataTargetReklame, RealisasiSDHariAccrual = dataRealisasiReklame, PersenAccrual = dataTargetReklame > 0 ? Math.Round((decimal)dataRealisasiReklame / dataTargetReklame * 100, 2) : 0 },
                        //new() { JenisPajak = EPajak.PBB.GetDescription(),            AkpTahun = dataTargetPbb, RealisasiSDHariAccrual = dataRealisasiPbb, PersenAccrual = dataTargetPbb > 0 ? Math.Round((decimal)dataRealisasiPbb / dataTargetPbb * 100, 2) : 0 },
                        new() { JenisPajak = EPajak.BPHTB.GetDescription(),          AkpTahun = dataTargetBphtb, RealisasiSDHariAccrual = dataRealisasiBphtb, PersenAccrual = dataTargetBphtb > 0 ? Math.Round((decimal)dataRealisasiBphtb / dataTargetBphtb * 100, 2) : 0 },
                        new() { JenisPajak = EPajak.AirTanah.GetDescription(),       AkpTahun = dataTargetAbt, RealisasiSDHariAccrual = dataRealisasiAbt, PersenAccrual = dataTargetAbt > 0 ? Math.Round((decimal)dataRealisasiAbt / dataTargetAbt * 100, 2) : 0 },
                        new() { JenisPajak = EPajak.OpsenPkb.GetDescription(),       AkpTahun = dataTargetOpsenPkb, RealisasiSDHariAccrual = dataRealisasiOpsenPkb, PersenAccrual = dataTargetOpsenPkb > 0 ? Math.Round((decimal)dataRealisasiOpsenPkb / dataTargetOpsenPkb * 100, 2) : 0 },
                        new() { JenisPajak = EPajak.OpsenBbnkb.GetDescription(),     AkpTahun = dataTargetOpsenBbnkb, RealisasiSDHariAccrual = dataRealisasiOpsenBbnkb, PersenAccrual = dataTargetOpsenBbnkb > 0 ? Math.Round((decimal)dataRealisasiOpsenBbnkb / dataTargetOpsenBbnkb * 100, 2) : 0 },
                    };
