...
namespace PbbWs
{
    public class PBBUptb2 : BackgroundService
    {
        private bool isFirst = true;
        private readonly ILogger<PBBUptb2> _logger;
        private static int KDPajak = 9;
        private static int Uptb = 2;

        public PBBUptb2(ILogger<PBBUptb2> logger)
        {
            _logger = logger;
        }
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            _logger.LogInformation("PBBUptb2 running at: {time}", DateTimeOffset.Now);
