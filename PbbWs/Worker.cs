    ...
    public class Worker : BackgroundService
    {
        ...
        private async Task DoWorkNewMeta(CancellationToken stoppingToken)
        {
            ...
            for (var i = tahunAmbil; i <= tglServer.Year; i++)
            {
                //GetKetetapan(i);
                await Task.WhenAll(
                   GetKetetapanAsync(i, 1),
                   GetKetetapanAsync(i, 2),
                   GetKetetapanAsync(i, 3),
                   GetKetetapanAsync(i, 4),
                   GetKetetapanAsync(i, 5)
                   );
            }
            //for (var i = tahunAmbil; i <= tglServer.Year; i++)
            //{
        ...
        public async Task GetOPProcessAsync(int uptb)
        {
            try
            {
        ...
        private async Task GetKetetapanAsync(int tahunBuku, int uptb)
        {
            try
            {
                Console.WriteLine($"Get Ketetapan UPTB :{uptb}");
...
LEFT JOIN PEMBAYARAN_SPPT@LIHATGATOTKACA D ON D.KD_PROPINSI=A.T_PROP_KD AND  D.KD_DATI2=A.T_DATI2_KD  AND  D.KD_KECAMATAN=A.T_KEC_KD  AND  D.KD_KELURAHAN = A.T_KEL_KD AND
    ...
    .GroupBy(x => new
    {
        x.NOP,        x.TAHUN_BUKU,        x.KATEGORI_ID,        x.KATEGORI_NAMA,        x.ALAMAT_OP,        x.ALAMAT_OP_NO,
        x.ALAMAT_OP_RT,        x.ALAMAT_OP_RW,        x.ALAMAT_KD_CAMAT,        x.ALAMAT_KD_LURAH,        x.UPTB,        x.ALAMAT_WP,
        x.ALAMAT_WP_NO,        x.ALAMAT_WP_KEL,        x.ALAMAT_WP_KOTA,        x.WP_NAMA,        x.WP_NPWP,        x.TAHUN_PAJAK,
        x.POKOK_PAJAK,        x.KATEGORI_OP,        x.PERUNTUKAN,        x.IS_LUNAS,        x.TGL_BAYAR,        x.INS_DATE,        x.INS_BY
    })
    .Select(g => new
    {
        NOP = g.Key.NOP,
