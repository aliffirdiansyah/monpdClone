...
namespace PbbWs
{
    public class PBBUptb5 : BackgroundService
    {
        private bool isFirst = true;
        private readonly ILogger<PBBUptb5> _logger;
        private static int KDPajak = 9;
        private static int Uptb = 5;

        public PBBUptb5(ILogger<PBBUptb5> logger)
        {
            _logger = logger;
        }
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            _logger.LogInformation("PBBUptb5 running at: {time}", DateTimeOffset.Now);
