...
using static EAccounting.Helper;
    ...
    public class Worker : BackgroundService
    {
        ...
        private async Task DoWorkFullScanAsync(CancellationToken stoppingToken)
        {
            ...
            try
            {
                ...
                client.BaseAddress = new Uri("https://eaccounting.surabaya.go.id");
                client.DefaultRequestHeaders.Add("username", "bapenda");
                client.DefaultRequestHeaders.Add("password", "Bapenda@sby2022");

                try
                {
                    // Kirim GET request
                    var response = await client.GetAsync("/api-eaccounting2025/mutasi");

                    // Cek sukses
                    response.EnsureSuccessStatusCode();

                    string jsonString = await response.Content.ReadAsStringAsync();
                    // Deserialisasi JSON ke List<MutasiResponse>
                    var options = new JsonSerializerOptions { PropertyNameCaseInsensitive = true };
                    List<MutasiResponse> data = JsonSerializer.Deserialize<List<MutasiResponse>>(jsonString, options);
                ...
                {
                    Console.WriteLine($"Terjadi kesalahan HTTP: {ex.Message}");
                }                
            }
            catch (Exception)
            {
