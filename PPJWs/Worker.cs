...
WHERE A.NPWPD NOT IN (
 ...
 GROUP BY A.NOP,A.TAHUN,A.MASAPAJAK,A.SEQ,A.TGL_SPTPD ,A.TGL_JATUH_TEMPO_BAYAR ,A.PROSEN_TARIF_PAJAK)";
                        ...
                        foreach (var itemKetetapan in ketetapanSbyTaxOld)
                        {
                            ...
                            var rowMonPpj = _contMonPd.DbMonPpjs.SingleOrDefault(x => x.Nop == nop && x.TahunPajakKetetapan == tahunPajak &&
                            ...
                            if (rowMonPpj != null)
                            {
                                _contMonPd.DbMonPpjs.Remove(rowMonPpj);
                            }
                    ...
                    Console.Write($"\rDB_OP_PPJ SBYTAX TAHUN {tahunBuku} JML OP {jmlData} : {item.Nop}  {persen:F2}%   ");
                    Thread.Sleep(50);
                    _contMonPd.SaveChanges();
...
WHERE NAMA_PAJAK_DAERAH='PPJ'  AND REPLACE(FK_NOP,'.','')=:NOP AND TO_CHAR(TGL_SETORAN,'YYYY')=:TAHUN ";
                    ...
                    new OracleParameter("TAHUN", tahunBuku)
                        ...
                        if (pembayaranSspdList != null)
                        {
                            foreach (var itemSSPD in pembayaranSspdList)
                            {
                                ...
                                if (ketetapan == null)
                                {
                                    ketetapan = _contMonPd.DbMonPpjs.SingleOrDefault(x => x.Nop == itemSSPD.NOP &&
                                                                                        x.TahunPajakKetetapan == itemSSPD.TAHUN &&
                                                                                        x.MasaPajakKetetapan == itemSSPD.MASA &&
                    ...
                    index++;
                    double persen = ((double)index / jmlData) * 100;
                    Console.Write($"\rDB_OP_PPJ HPP TAHUN {tahunBuku} JML OP {jmlData} : {item.Nop}  {persen:F2}%   ");
                    Thread.Sleep(50);
                    _contMonPd.SaveChanges();
        ...
        //        '-'  NAMA_SUB_RINCIAN    
        //FROM OBJEK_PAJAK A
        //JOIN OBJEK_PAJAK_PPJ B ON A.NOP = B.NOP
        //JOIN NPWPD C ON A.NPWPD = C.NPWPD_no
        //JOIN M_KATEGORI_PAJAK D ON D.ID = A.KATEGORI
        //LEFT JOIN M_KECAMATAN B ON A.KD_CAMAT = B.KD_CAMAT
        //WHERE A.NPWPD NOT IN (
        ...
        //                    var result = await _contSbyTax.Set<DbOpListrik>().FromSqlRaw(sql, new[] {
        ...
        //		NVL(B.KATEGORI_NAMA, 'PPJ') KATEGORI_NAMA,
        //	    NAMA_AYAT_PAJAK,
        //	        0 METODE_PENJUALAN,
        //	        0 METODE_PEMBAYARAN,
        //	        0 JUMLAH_KARYAWAN,
        ...
        //			        WHEN KATEGORI_STATUS_BARU = 'RESTORAN PADA OBJEK PPJ' THEN 9
        ...
        //			        WHEN KATEGORI_STATUS_BARU = 'USAHA PPJ' THEN 37
        ...
        //			        WHEN KATEGORI_STATUS_BARU = 'USAHA RESTORAN/PPJ' THEN 53
        ...
        //						WHEN KATEGORI_STATUS = 'USAHA RESTORAN' THEN 'USAHA RESTORAN/PPJ'
        //						ELSE KATEGORI_STATUS
        //					END AS KATEGORI_STATUS_BARU
        //			FROM T_OP_KATEGORI_STATUS
        ...
        //				WHERE NAMA_PAJAK_DAERAH = 'PPJ' AND STATUS_OP = 1 AND KATEGORI_PAJAK != 'INSIDENTIL'
        //			) A
        //		) A
        //	) A
        //	WHERE PAJAK_ID = 5
        ...
        //                    var result = await _contMonitoringDb2.Set<DbOpListrik>().FromSqlRaw(sql).ToListAsync();
        ...
        //                        }
        //                        Console.WriteLine($"{DateTime.Now} [QUERY] KETETAPAN SBYTAX PPJ {thn}-{bln}-{op.Nop}");
        //                        var sql = @"
        //                            SELECT 	A.NOP,
        //                              A.TAHUN,
        //                              A.MASAPAJAK,
        ...
        //                            }).ToListAsync();
        //                        Console.WriteLine($"{DateTime.Now} [QUERY_FINISHED] KETETAPAN SBYTAX PPJ {thn}-{bln}-{op.Nop}");
        ...
        //                            var rowMonHiburan = _contMonPd.DbMonPpjs.SingleOrDefault(x => x.Nop == nop && x.TahunPajakKetetapan == tahunPajak &&
        //                                                                                    x.MasaPajakKetetapan == masaPajak && x.SeqPajakKetetapan == seqPajak);

        //                            if (rowMonHiburan != null)
        //                            {
        //                                _contMonPd.DbMonPpjs.Remove(rowMonHiburan);
        //                            }
        ...
        //                            Console.WriteLine($"DB_MON_PPJ {thn}-{bln}-{item.NOP}-{item.SEQ}");
        //                            Console.ResetColor();
        //                        }
        //                    }
        //                }
        ...
        //        FROM VW_SIMPADA_SSPD@LIHATHPPSERVER
        //        WHERE NAMA_PAJAK_DAERAH='PPJ' AND TAHUN_SETOR=:TAHUN AND TO_NUMBER(TAHUN_PAJAK) = :TAHUN
        //    ) A
        //                ";
        ...
        //  	                            SELECT *
        //                                FROM VW_SIMPADA_SPTPD@LIHATHPPSERVER
        //                                WHERE NAMA_PAJAK_DAERAH='PPJ' AND FK_NOP IS NOT NULL AND TO_NUMBER(TAHUN_PAJAK) = :tahun AND BULAN_PAJAK = :bulan
        //                             ) A
        //                        ";
        ...
        //                            var rowMonHiburan = _contMonPd.DbMonPpjs
        //                                .SingleOrDefault(x =>
        //                                x.Nop == nop &&
        ...
        //                            _contMonPd.SaveChanges();
        //                            Console.ForegroundColor = ConsoleColor.Green;
        //                            Console.WriteLine($"{DateTime.Now} DB_MON_PPJ_MONITORINGDB {thn}-{bln}-{item.NOP}-{item.SEQ}");
        //                            Console.ResetColor();
        //                        }
        //                    }
        //                }
        ...
        //            }
        //        }
        //        private void GetRealisasiPhr(ref DbMonPpj row, List<SSPDPbjt> pembayaranSspdList)
        //        {
        //            //PEMBAYARAN PHR
