...
WHERE 	NAMA_PAJAK_DAERAH ='PARKIR' 
        ...
        private void GetRealisasi(ref DbMonParkir row)
        {
            //PEMBAYARAN
            ...
            if (pembayaranSspdList != null && pembayaranSspdList.Count > 0)
            {
                ...
                Console.WriteLine($"DB_MON_PARKIR (SSPD): {row.TahunPajakKetetapan}-{row.MasaPajakKetetapan}-{row.Nop}-{row.SeqPajakKetetapan}-{row.NominalPokokBayar}");
            }
        }
        private void GetRealisasiPhr(ref DbMonParkir row)
        {
            //PEMBAYARAN PHR
            var _contHpp = DBClass.GetHppContext();
            ...
            var sql = @"
                SELECT 	REPLACE(FK_NOP, '.', '') NOP,
	                    ...
	                    0 PENGURANG_SANSKSI,
	                    100 SEQ_KETETAPAN
	            FROM VW_SIMPADA_SSPD A
	            WHERE NAMA_PAJAK_DAERAH='PARKIR' AND TAHUN_SETOR=:TAHUN AND REPLACE(FK_NOP, '.', '') = :NOP AND TO_NUMBER(TAHUN_PAJAK) = :TAHUN AND BULAN_PAJAK = :MASA
            ";

            var pembayaranSspdList = _contHpp.Set<SSPDPbjt>()
                 .FromSqlRaw(sql, new[] {
                        new OracleParameter("NOP", row.Nop),
                        new OracleParameter("TAHUN", row.TahunPajakKetetapan),
                        new OracleParameter("MASA", row.MasaPajakKetetapan),
            ...
            if (pembayaranSspdList != null)
            {
                ...
                if (pembayaranSspdList.Count > 0)
                {
                    ...
                    Console.ForegroundColor = ConsoleColor.Green;
                    Console.WriteLine($"{DateTime.Now} [SAVED] DB_MON_PARKIR (SSPD) (PHR): {row.TahunPajakKetetapan}-{row.MasaPajakKetetapan}-{row.Nop}-{row.SeqPajakKetetapan}");
                    Console.ResetColor();
                }
            }
        }
