...
LEFT JOIN M_KECAMATAN B ON A.KD_CAMAT = B.KD_CAMAT        
                ...
                using (var _contHpp = DBClass.GetHppContext())
                {
                    ...
                    var result = await _contHpp.Set<DbOpParkir>().FromSqlRaw(sql).ToListAsync();
                ...
                for (int bln = 1; bln <= 12; bln++)
                {
                    ...
                    foreach (var op in opList)
                    {
                        ...
                        foreach (var item in ketetapanSbyTaxOld.Where(x => x.NOP == op.Nop))
                        {
                            ...
                            Console.WriteLine($"DB_MON_PARKIR_MONITORINGDB {thn}-{bln}-{item.NOP}-{item.SEQ}");
                            Console.ResetColor();
                        }
                    }
                }
                ...
                for (int bln = 1; bln <= 12; bln++)
                {
                    ...
                    foreach (var op in opList)
                    {
                        ...
                        if (pembayaranSspdList1 != null && pembayaranSspdList1.Count > 0)
                        {
                            ...
                            _contMonPd.SaveChanges();
                            Console.WriteLine($"DB_MON_PARKIR (SSPD): {thn}-{bln}-{op.Nop}-{op.SeqPajakKetetapan}");
                        }
                    }
                }
            }
