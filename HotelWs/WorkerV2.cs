...
namespace HotelWs
{
    public class WorkerV2 : BackgroundService
    {
        ...
        private static EnumFactory.EPajak PAJAK_ENUM = EnumFactory.EPajak.JasaPerhotelan;
...
WHERE fk_pajak_daerah = '01' AND status_op != 0
)
                    ";
                ...
                var removeEx = _contMonPd.DbOpHotels.Where(x => x.TahunBuku == tahunBuku).ToList();
                foreach (var item in result)
                {
                    ...
                    Console.Write($"\r[{tglMulai.ToString("dd MMM yyyy HH:mm:ss")}] OP HOTEL TAHUN {tahunBuku} JML OP {jmlData.ToString("n0")} [({persen:F2}%)]");
                }
                ...
                if (removeEx.Any())
                {
                    _contMonPd.DbOpHotels.RemoveRange(removeEx);
                ...
                foreach (var itemRealisasi in realisasiMonitoringDb)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            newRowOP.PajakNama = "Pajak Jasa Perhotelan";
                            newRowOP.NamaOp = "-";
                            newRowOP.AlamatOp = "-";
                            newRowOP.AlamatOpNo = "-";
                            newRowOP.AlamatOpRt = "-";
        ...
        private void UpdateKoreksi(int tahunBuku)
        {
            ...
            if (!isTahunSekarang)
            {
                ...
                if (result.Count > 0)
                {
                    ...
                    source = context.DbOpHotels.FirstOrDefault(x => x.Nop == nop && x.TahunBuku == tahunBuku);
                    if (source == null)
                    {
