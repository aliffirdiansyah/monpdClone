...
namespace HotelWs
{
    public class Worker : BackgroundService
    {
        ...
        private void GetRealisasi(ref DbMonHotel row)
        {
            //PEMBAYARAN
            ...
            if (pembayaranSspdList != null && pembayaranSspdList.Count > 0)
            {
                ...
                Console.WriteLine($"DB_MON_HOTEL (SSPD): {row.TahunPajakKetetapan}-{row.MasaPajakKetetapan}-{row.Nop}-{row.SeqPajakKetetapan}-{row.NominalPokokBayar}");
            }
        }
        private void GetRealisasiPhr(ref DbMonHotel row)
        {
            //PEMBAYARAN PHR
