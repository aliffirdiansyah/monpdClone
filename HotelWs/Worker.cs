...
LEFT JOIN M_KECAMATAN B ON A.KD_CAMAT = B.KD_CAMAT    
            ...
            ";
            var pembayaranSspdList = await _contPhr.Set<SSPDPbjt>().FromSqlRaw(sql1).ToListAsync();
            ...
            for (var thn = tahunAmbil; thn <= tglServer.Year; thn++)
            {
                for (int bln = 1; bln <= 12; bln++)
                {
                    ...
                    foreach (var op in opList)
                    {
                        ...
                        {
                            pembayaranSspdList = pembayaranSspdList.Where(x => x.TAHUN_PAJAK == thn && x.BULAN_PAJAK == bln && x.NOP == op.Nop).ToList();
