...
namespace DBPendapatanDaerahWs
{
    public class Worker : BackgroundService
    {
        ...
        private async Task DoWorkFullScanAsync(CancellationToken stoppingToken)
        {
            ...
            for (var thn = tahunAmbil; thn <= tglServer.Year; thn++)
            {
                ...
                string url = $"https://esidatra.surabaya.go.id/api/MonPdRealisasiPendapatanDaerah{thn}";
                using (HttpClient httpClient = new HttpClient())
                {
                    ...
                    if (result != null)
                    {
                        if (result.Count > 0)
                        {
                            ...
                            var dataExisting = _contMonPd.DbPendapatanDaerahs.Where(x => x.TahunBuku == thn).ToList();
                            _contMonPd.DbPendapatanDaerahs.RemoveRange(dataExisting);

                            int index = 0;
                            int jmlData = result.Count;
                            foreach (var item in result)
                            {
                                var res = new DbPendapatanDaerah();
                                ...
                                _contMonPd.DbPendapatanDaerahs.Add(res);
